package com.hristodaskalov;

import java.util.*;

public class Theatre {
    private final String theatreName;
    private List<Seat> seats = new ArrayList<>();

    public Theatre(String theatreName, int numRows, int seatsPerRow) {
        this.theatreName = theatreName;

        int lastRow = 'A' + (numRows - 1);
        for (char row = 'A'; row <= lastRow; row++) {
            for(int seatNum = 1; seatNum <= seatsPerRow; seatNum++) {
                Seat seat = new Seat(row + String.format("%02d", seatNum));
                seats.add(seat);
            }
        }
    }

    public String getTheatreName() {
        return theatreName;
    }

    private int searchSeat(String seatNumber) {
        int low = 0; // implementing binary type search for the seats
        int high = seats.size()-1;

        while (low <= high) {

            int mid = (low + high) / 2;
            Seat midVal = seats.get(mid);
            int cmp = midVal.getSeatNumber().compareTo(seatNumber);

            if (cmp < 0) {
                low = mid + 1;
            } else if (cmp > 0) {
                high = mid - 1;
            } else {
                return mid;
            }
        }
        return -1;
    }

    public void reserveSeat(String seat){

        int result = searchSeat(seat);
        if(result == -1){
            System.out.println("There is no seat " + seat);
        } else{
            seats.get(result).reserve();

        }
    }

    public void cancelSeat(String seat){

        int result = searchSeat(seat);
        if(result == -1){
            System.out.println("There is no seat " + seat);
        } else{
            seats.get(result).cancel();
        }
    }

    // for testing
    public void getFreeSeats() {
        for(Seat seat : seats) {
            if(!seat.reserved) {
                System.out.println(seat.getSeatNumber());
            }
        }
    }

    private class Seat implements Comparable<Seat> {
        private final String seatNumber;
        private boolean reserved = false;

        private Seat(String seatNumber) {
            this.seatNumber = seatNumber;
        }

        @Override
        public int compareTo(Seat seat) {
            return this.seatNumber.compareToIgnoreCase(seat.getSeatNumber());
        }

        private boolean reserve() {
            if(!this.reserved) {
                this.reserved = true;
                System.out.println("Seat " + seatNumber + " reserved");
                return true;
            } else {
                return false;
            }
        }

        private boolean cancel() {
            if(this.reserved) {
                this.reserved = false;
                System.out.println("Reservation of seat " + seatNumber + " cancelled");
                return true;
            } else {
                return false;
            }
        }

        private String getSeatNumber() {
            return seatNumber;
        }
    }



















}
